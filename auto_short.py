...
def pulse_train(v_sweep, 
    ...
    global pulse_idx, meas_idx, currents, e_stop, loop_break
    loop_break = False
    e_stop = False
    ...
    for ii, n in enumerate(num_bursts):
        ...
        for jj in range(n):
            ...
            # Break loop if currents are too high
            print(currents[pulse_idx, kk])
            if currents[pulse_idx, kk] < -1e-9:
                loop_break = True
                print("Exited measurement due to current being too high: ", currents[pulse_idx,kk])
                break
            pulse_idx += 1
    LNA.set_bias(0)
...
def save_data():
    ...
    title = f'{device_name}_{device_idx}'
...
voltage_range = np.linspace(13, 23, num = 51)
string_voltage_range = np.array2string(voltage_range, separator = ',')
pulse_v_val.set(string_voltage_range[1:-1])
# Pulse period
...
pulse_width_repeat = 51*'100e-6,'
pulse_period_val.set(pulse_width_repeat[:-1])
# Pulse per Burst
...
p_per_b_repeat = 51*'1,'
pulse_per_burst_val.set(p_per_b_repeat[:-1])
# Number of Bursts
...
pulse_num_repeat = 51*'2,'
pulse_num_burst_val.set(pulse_num_repeat[:-1])
